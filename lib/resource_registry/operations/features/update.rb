    ...
    module Features
      ...
      class Update
        ...
        def call(params)
          ...
          yield save_record(entity, params[:registry], params[:filter]) if defined? Rails
          yield update_registry(entity, params[:registry])
        ...
        def validate(feature_params, registry)
          ...
          if date_range_settings.present?
            feature_params[:settings].each do |setting_hash|
              next unless (date_range_setting = date_range_settings.detect{|s| s.key == setting_hash[:key].to_sym})
